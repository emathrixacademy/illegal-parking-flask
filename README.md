...
```python
CAM1_URL = "rtsp://admin:%40Project001@192.168.1.3:554/stream1"  # VIGI cam (DHCP, IP may change)
...
|--------|------|-----|--------|------|
| CAM1 (MAIN) | TP-Link VIGI | 192.168.1.3 (DHCP) | 192.168.1.x | admin / @Project001 |
...
CAM3 (ONVIF, 192.168.8.x subnet — no auth for RTSP):
  IP:   192.168.8.199
  RTSP: rtsp://192.168.8.199:554/stream

ONVIF admin panels (CAM2 & CAM3 only, NOT for RTSP):
  User: admin
  Pass: admin123
```
...
API Key: dcgl-pi-secret-2026 (sent as X-API-Key header)
```

### Detection Models
- **yolov8s.hef** — Vehicle detection (COCO: person, bike, car, motorcycle, bus, truck)
- **cctv_ai.pt** — Custom trained (11 classes: bike, car, chair, jeep, motorcycle, rock, trash, tree, tricycle, vendor, background)
- **cctv_ai_reserved.pt** — Previous model backup
- CCTV AI detection threshold: `0.1` (lowered for better trash sensitivity)
- Claude Vision garbage detection: **disabled** (API credits exhausted)
- Scene-change tamper detection: **disabled** (false positives from outdoor lighting)
...
### Cloudinary Management API (Admin only)
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/cloudinary_toggle` | GET | Check if uploads are paused |
| `/api/cloudinary_toggle` | POST | Pause/resume Cloudinary uploads |
| `/api/cloudinary_cleanup` | POST | Trigger 14-day auto-cleanup manually |
| `/api/cloudinary_media` | GET | List violations with cloud media (`?filter=all\|7\|14\|30`) |
| `/api/cloudinary_delete_selected` | POST | Delete Cloudinary assets for selected violation IDs |
| `/api/cloudinary_test` | GET | Test Cloudinary connectivity |
...
**Commit `054ac42`** (May 17, 2026) — fully working and verified in production.
...
- Cloudinary 14-day auto-cleanup (background thread every 6h)
- Cloud Media Uploads pause/resume toggle in Settings
- Cloud Media Management panel (browse, select, delete individual assets)
- Philippine plate format validation (rejects OCR garbage)
- Pi network reliability deploy files (static IP, camera subnet service, watchdog)
- Pi SSH: `admin` / `admin123`

## Changelog

### May 17, 2026 - Cloudinary Cost Control, Plate Validation & Pi Reliability
- **Cloudinary Auto-Cleanup**: Background thread deletes Cloudinary assets older than 14 days (runs every 6 hours), keeps violation records in DB
- **Cloud Media Uploads Toggle**: Admin can pause/resume Cloudinary uploads via Settings to save credits when demo is not near
- **Cloud Media Management Panel**: Browse violations with cloud media, filter by age (7/14/30 days), select individual items to keep or delete with thumbnail previews
- **Manual Cleanup API**: `POST /api/cloudinary_cleanup` for on-demand bulk cleanup, `POST /api/cloudinary_delete_selected` for selective deletion
- **Philippine Plate Validation**: OCR now rejects non-PH plate formats (must match 2-3 letters + 3-4 digits pattern, e.g., "NAZ 312"); garbage like "53511703" or "BIK-E{46098" is filtered out
- **Pi Network Reliability Files**: `pi-deploy/` folder with systemd units for static IP, camera subnet auto-add on boot, and network watchdog with auto-recovery
- **Cloudinary Plan**: Upgraded to Small PAYG ($29/month, 60 credits) from exhausted free tier
